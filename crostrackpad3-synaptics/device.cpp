	...
	)
{
	...
	sprintf(sc->firmware_version, "%ld", pDevice->firmware_id);

	sc->resx = pDevice->x_size_mm * 10;
	sc->resy = pDevice->y_size_mm * 10;

	sc->phyx = pDevice->max_x;
	sc->phyy = pDevice->max_y;
...
--*/
{
	...
	case IOCTL_HID_GET_DEVICE_DESCRIPTOR:
		...
		//
		status = SynaGetHidDescriptor(device, FxRequest);
		fSync = TRUE;
		break;
	...
	case IOCTL_HID_GET_DEVICE_ATTRIBUTES:
		...
		//
		status = SynaGetDeviceAttributes(FxRequest);
		fSync = TRUE;
		break;
	...
	case IOCTL_HID_GET_REPORT_DESCRIPTOR:
		...
		//
		status = SynaGetReportDescriptor(device, FxRequest);
		fSync = TRUE;
		break;
	...
	case IOCTL_HID_GET_STRING:
		...
		// the device.
		//
		status = SynaGetString(FxRequest);
		fSync = TRUE;
		break;
	...
	case IOCTL_HID_SET_OUTPUT_REPORT:
		...
		//
		status = SynaWriteReport(pDevice, FxRequest);
		fSync = TRUE;
		break;
	...
	case IOCTL_HID_GET_INPUT_REPORT:
		...
		// 
		status = SynaReadReport(pDevice, FxRequest, &fSync);
		break;
	...
	case IOCTL_HID_GET_FEATURE:
		...
		//
		status = SynaGetFeature(pDevice, FxRequest, &fSync);
		break;
